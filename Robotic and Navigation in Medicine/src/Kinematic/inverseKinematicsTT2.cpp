#include "inverseKinematicsTT2.h"
#include <math.h>
...
#define PI boost::math::constants::pi<float>()
  ...
  std::array<std::array<float, 8>, 6> InverseKinematics::computeInverseKinematics (boost::numeric::ublas::matrix<float> endPose) {
    ...
    handPos [0] = endPose (0, 3) - d[5] * z [0];
    handPos [1] = endPose (1, 3) - d[5] * z [1];
    handPos [2] = endPose (2, 3) - d[5] * z [2];
    ...
    theta_5[0] = acos (
        (endPose (0, 3) * sin (theta_1[0]) - endPose (1, 3) * cos (theta_1[0]) - d[3]) / d[5]);
    theta_5[1] = -acos (
        (endPose (0, 3) * sin (theta_1[1]) - endPose (1, 3) * cos (theta_1[1]) - d[3]) / d[5]);
